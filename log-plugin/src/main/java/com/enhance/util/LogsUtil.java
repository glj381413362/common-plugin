...
//      Profiler childProfiler = profilerRegistry.get(name);
   ...
   *         LogsUtil.printCode("orderEntry skuid和数量", "-", orderEntry.getSkuId().toString(),orderEntry.getQuantity().toString());
   *         log.info("开始处理...");
   *         log.info("处理中");
   *         log.info("处理结束");
   *         LogsUtil.clearMDC();
   ...
   * for (OrderEntry orderEntry : orderEntries) {
   *         LogsUtil.printCode("处理的skuid", orderEntry.getSkuId().toString());
   *         log.info("开始处理...");
   *         log.info("处理中");
   *         log.info("处理结束");
   *         LogsUtil.clearMDC();
   ...
   * 例子：orderEntries 有两条数据
   * for (OrderEntry orderEntry : orderEntries) {
   *         LogsUtil.printCode(orderEntry.getSkuId().toString());
   *         log.info("开始处理...");
   *         log.info("处理中");
   *         log.info("处理结束");
   *         LogsUtil.clearMDC();
   ...
   * 例子：orderEntries 有两条数据
   * for (OrderEntry orderEntry : orderEntries) {
   *         LogsUtil.printCodeByTemplate("skuId[{}] 数量[{}]",orderEntry.getSkuId(),orderEntry.getQuantity());
   *         log.info("开始处理...");
   *         log.info("处理中");
   *         log.info("处理结束");
   *         LogsUtil.clearMDC();
